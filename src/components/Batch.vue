...
watch(batchQuery.value, () => {
  if (resources.indexOf("Operations-Batch-Query") > -1) {
    handelList();
  }
});
const batchModify = ref({
  id: "",
  courseId: [],
  batchName: "",
  startToEnd: [],
...
const showDrawer = (type: string, title: string, data?: any) => {
  ...
  if (type === "edit") {
    nextTick(() => {
      batchModify.value = JSON.parse(JSON.stringify(data));
      console.log(data);
    });
  }
};
...
const handelCasChange = (value: any[]) => {
  const ids = [];
  for (let i = 0; i < value.length; i++) {
    const id = toRaw(value[i])[0];
    ids.push(id);
  }
  batchModify.value.courseId = ids;
};
...
<template>
  ...
  >
    <div style="display: flex; width: 200px; margin-top: 12px">
      <label
        ...
        >批次名</label
      >
      <el-input v-model="batchQuery.batchName" placeholder="请输入" clearable />
    </div>
    <div style="display: flex; width: 400px; margin-top: 12px">
      <label
    ...
    <div style="display: flex; padding-left: 12px; margin-top: 12px">
      <el-button
        v-if="resources.indexOf('Operations-Batch-Query') > -1"
        type="primary"
        @click="handelList"
        :loading="loading"
        >搜索</el-button
  ...
  >
    <el-row>
      <el-col :span="4">
        <div style="line-height: 32px" ref="titleColRef">批次列表</div>
      </el-col>
      <el-col :span="20" style="text-align: right">
        <el-button
          v-if="resources.indexOf('Operations-Batch-Add') > -1"
          type="primary"
          @click="showDrawer('add', '新增批次')"
        >
          新增批次
        </el-button>
      </el-col>
    ...
    >
      ...
      <el-table-column label="状态" prop="status">
        <template
          #default="scope"
          v-if="resources.indexOf('Operations-Batch-Modify') > -1"
        >
      ...
      <el-table-column
        ...
        width="90"
        v-if="
          resources.indexOf('Operations-Batch-Modify') > -1 ||
          resources.indexOf('Operations-Batch-Remove') > -1
        "
      >
        <template #default="scope">
          <el-breadcrumb separator="|">
            ...
            <el-tooltip
              v-if="resources.indexOf('Operations-Batch-Remove') > -1"
              class="box-item"
              effect="dark"
              content="删除"
              placement="top"
    ...
    <el-row v-if="resources.indexOf('Operations-Batch-Query') > -1">
      <el-col :span="24">
    ...
    <template #default>
      ...
      >
        <el-form-item label="批次名" prop="batchName">
          <el-input v-model="batchModify.batchName" placeholder="请输入" />
        </el-form-item>

        <el-form-item label="课程" prop="courseId">
          <el-cascader
            v-model="batchModify.courseId"
            placeholder="请选择课程"
            :options="courseOption"
            :props="prop"
            @change="handelCasChange"
            filterable
          />
        </el-form-item>
        ...
        <el-form-item label="起止时间">
          <el-date-picker
            v-model="batchModify.startToEnd"
            type="datetimerange"
            value-format="YYYY-MM-DD hh:mm:ss"
            range-separator="至"
            start-placeholder="开始时间"
    ...
    <template #footer>
      <div style="flex: auto">
        ...
        <el-button
          ...
          >提交</el-button
        >
        <el-button type="primary" @click="handelModify(batchModify)" v-else
          >修改</el-button
        >
      </div>
    </template>
